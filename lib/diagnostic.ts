 ...
 * The kid sees an adventure. The parent gets a map of exactly which bits are
 * known, shaky, or unknown, and the path is built from that map.
 */
...
export const BANDS_WORDS: Band[] = [
  {
    ...
    probes: A_FIRST_SOUNDS.map(({ ph, foils }, i) =>
  ...
  },
  {
    ...
    probes: A_LETTER_NAMES.map(({ letter, decoys }, i) =>
  ...
  },
  {
    ...
    probes: A_LETTER_SOUNDS.map(({ letter, decoys }, i) =>
  ...
  },
  {
    ...
    probes: (["a", "e", "i", "o", "u"] as Vowel[]).map((v, i) => {
...
export const BANDS_LETTERS: Band[] = [
  ...
  },
  {
    ...
    dimensions: ["phonemes", "speaking"],
    probes: A_FIRST_SOUNDS.slice(0, 4).map(({ ph, foils }, i) =>
...
export function bandShouldStop(rows: DiagnosticRow[]) {
  ...
  if (rows.length >= 4 && rows.filter((r) => r.ok).length < 2) return true;
  return false;
}

export function bandStatus(band: Band, rows: DiagnosticRow[]): BandStatus {
  if (rows.length === 0) return "not-reached";
  const hits = rows.filter((r) => r.ok).length;
  if (hits >= Math.ceil(band.probes.length * 0.8)) return "known";
  if (hits >= 2 || hits / rows.length >= 0.5) return "shaky";
  return "unknown";
}
...
export function answerProbe(
  ...
  const row: DiagnosticRow = { probeId: probe.id, band: probe.band, bit: probe.bit, ok, ms, at: now.toISOString() };
...
export function bandReports(progress: DiagnosticProgress): BandReport[] {
  ...
  return bands.map((band) => {
    const rows = progress.rows.filter((r) => r.band === band.id);
    return {
      id: band.id,
      title: band.title,
      status: bandStatus(band, rows),
      hits: rows.filter((r) => r.ok).length,
      answered: rows.length,
      total: band.probes.length,
      known: rows.filter((r) => r.ok).map((r) => r.bit),
      missed: rows.filter((r) => !r.ok).map((r) => r.bit),
    };
  });
}
...
function targetedModule(kidId: string, band: Band, focus: string[], review: string[], stamp: string): ModuleDef {
  const items: LessonItem[] = [];
  ...
  return {
    id: `dx-${kidId}-${band.id}-${stamp}`,
    title: `${band.title}: ${focus.join(", ")}`,
    track: band.id.startsWith("my-") ? "letters" : "words",
    skill: band.teaches,
    stretch: "stretch-hard",
    dimensions: band.dimensions,
    items: unique.length ? unique : band.probes.slice(0, 3).map((p) => ({ ...p, kind: "tap" as const })),
    custom: true,
  };
}
...
export function buildPath(kidId: string, progress: DiagnosticProgress, reports: BandReport[], stamp = Date.now().toString(36)): BuiltPath {
  ...
  for (let i = 0; i < bands.length; i++) {
    ...
    if (r.status === "unknown") {
      if (band.bankModule) push(band.bankModule);
      const allBits = band.probes.map((p) => p.bit);
...
export function finishDiagnostic(kidId: string, progress: DiagnosticProgress, stamp?: string): { report: DiagnosticReport; built: BuiltPath } {
  ...
  const frontier = reports.find((b) => b.status !== "known" && b.status !== "not-reached")?.id;
  const known = reports.filter((b) => b.status === "known").map((b) => b.title);
  const shaky = reports.filter((b) => b.status === "shaky");
  const unknown = reports.find((b) => b.status === "unknown");
  const parts: string[] = [];
  ...
  if (unknown) parts.push(`Frontier: ${unknown.title.toLowerCase()} (${unknown.hits}/${unknown.answered}).`);
  if (!known.length && !shaky.length && !unknown) parts.push("No bands answered.");
  parts.push(`Built ${builtPath.modules.length} targeted module${builtPath.modules.length === 1 ? "" : "s"}; ${builtPath.passed.length} bank module${builtPath.passed.length === 1 ? "" : "s"} marked passed.`);
